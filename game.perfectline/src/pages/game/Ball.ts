...
class Ball extends Laya.Sprite{
    ...
    private _roleData:{skin:string, shadow:string, rotate:boolean,rendFrame?:number};
    ...
    public setSkin(id:any, speed:number):void{
        //if(this._id != id){
            var info:{skin:string, shadow:string, rotate:boolean,rendFrame?:number} = DBGame.roleInfo[id];
            this._roleData = info;
            if(info){
