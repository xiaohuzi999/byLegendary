class GameActivity extends xframe.XWindow {
    ...
    private _data: { cid: any, id: string, json: string, mp3: string, name: string, stars: string };
    //渲染资源列表
    ...
    init() {
        ...
        this.ui.btnSelSong.on(Laya.Event.CLICK, null, () => {
            XFacade.instance.showModule(LevelsActivity, this.params)
        ...
        switch (type) {
            ...
            case GameEvent.SELECTED:
                ...
                if (record) {
                    ...
                    var list: Array<any> = GameDataManager.instance.getMuicList(this.params);
    ...
    private initMap(firstTime: boolean = true): void {
        ...
        var cfg: { speed: number, mp3: string, nodes: { x: number, y: number, sx: number, t: number }[], items: any } = Laya.loader.getRes('res/snd/' + this._data.json + '.json');
    ...
    private start(): void {
        ...
        var url: string = encodeURI(Laya.URL.basePath + GameActivity.mp3);
